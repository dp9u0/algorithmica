...
在[上一章](../external-memory)中，我们从理论的角度研究了计算机内存，用[外部存储器模型](../external-memory/model)来估算访存受限（memory-bound）算法的性能。

外部存储器模型对于涉及 HDD 和网络存储的计算来说或多或少是准确的——在那里，外部 I/O 操作的开销让针对内存中数值的算术操作可以忽略不计——但对于缓存层级中更靠下的层，它就太不精确了，因为在那里这两类操作的开销变得可以相互比拟。

为了对内存算法做更细粒度的优化，我们必须开始把 CPU 缓存系统的许多具体细节纳入考虑。与其去啃大量枯燥的 Intel 文档——满是干巴巴的规格说明和理论上可达的极限——我们将通过运行大量小型基准测试程序来实验性地估计这些参数，这些程序的访问模式模拟实际代码中经常出现的那类模式。
...
和之前一样，我将在 Ryzen 7 4700U 上运行所有实验。这是一颗 "Zen 2" CPU，缓存相关的主要规格如下：

- 8 个物理核心（无超线程），频率 2GHz（睿频模式 4.1GHz——[我们把它关了](/hpc/profiling/noise)）；
- 256K 的 8 路组相联 L1 数据缓存，即每核 32K；
- 4M 的 8 路组相联 L2 缓存，即每核 512K；
- 8M 的 16 路组相联 L3 缓存，由 8 个核心[共享](sharing)；
- 16GB（2x8G）DDR4 内存 @ 2667MHz。

在 Linux 上运行 `dmidecode -t cache` 或 `lshw -class memory`，或在 Windows 上安装 [CPU-Z](https://en.wikipedia.org/wiki/CPU-Z)，就可以拿它与你自己的硬件做个比较。关于这颗 CPU 的更多细节还可以在 [WikiChip](https://en.wikichip.org/wiki/amd/ryzen_7/4700u) 和 [7-CPU](https://www.7-cpu.com/cpu/Zen2.html) 上找到。并非所有结论都能推广到现存的所有 CPU 平台。
...
由于很难[阻止编译器优化掉未使用的值](/hpc/profiling/noise/)，本文中的代码片段为了讲解清晰而略有简化。如果你想自己复现它们，请查看[代码仓库](https://github.com/sslotin/amh-code/tree/main/cpu-cache)。

### 致谢 {#acknowledgements}

本章的灵感来自 Igor Ostrovsky 的 "[Gallery of Processor Cache Effects](http://igoro.com/archive/gallery-of-processor-cache-effects/)" 与 Ulrich Drepper 的 "[What Every Programmer Should Know About Memory](https://people.freebsd.org/~lstewart/articles/cpumemory.pdf)"，两者都可以作为很好的配套读物。
