...
> **译者注**：原文写于 2021–2022 年，M1 刚问世；如今 Apple 全线 Mac 均已采用自研 Arm 芯片（M 系列）——如果你用的就是 Apple Silicon Mac，你的 CPU 即本节所说的"桌面机例外"（AArch64）。Apple 选 Arm 是为复用 iPhone A 系列芯片的设计积累、摆脱 Intel 的路线图、换取更高的性能功耗比；但 M 系列之强并不在"Arm 指令集"本身——正如本节所述，ISA 只是接口，性能来自实现：宽译码的微架构、台积电最新制程、统一内存设计。另：本书后续汇编示例均为 x86；Arm 上的概念基本通用，可在 [Godbolt](https://godbolt.org/) 选 x86-64 编译器对照查看。
